...
<div id="main">
  <%@ include file="console_element/left.html" %>
   ...
   <% List<newspaper> unpd = (List)request.getAttribute("usernewspaperdata");%>
        <div id="opt_area">
    <h2 align="center">用户发布新闻</h2>
     <table id="blocks" class="table table-striped" style="margin-top:25px">
        <tr>
            ...
            <th class="hidden-phone">删除发布的新闻</th>
        </tr>
		<%for(int i = 0;i < unpd.size(); i++ ){ %>
        <tr>
            <td><a href="UserServlet?opr=jumpupnews&isedit=1&newsid=<%=unpd.get(i).newsid%>"><%=unpd.get(i).newstitle %></font></td>
            <td><%=unpd.get(i).newstime %></td>

            <td class="hidden-phone"><%=unpd.get(i).newstheme %></td>
            <td class="hidden-phone"><a href="UserServlet?opr=delnews&newsid=<%=unpd.get(i).newsid%>">删除</a></td>
        </tr>
        <%} %>
        </table>
