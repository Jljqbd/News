...
<div id="main">
  <%@ include file="console_element/left.html" %>
   <div class="container" align="center" >
    <% List<news_container> nc1 = (List)request.getAttribute("usercollectlist");%>
    <% List<String> authorname = (List)request.getAttribute("authorname"); %>
        <div id="opt_area">
    <h2 align="center">用户收藏的新闻</h2>
     <table id="blocks" class="table table-striped" style="margin-top:25px">
        <tr>
            <th>新闻标题</th>
            <th>新闻发布时间</th>

            <th class="hidden-phone">主题</th>
            <th class="hidden-phone">作者</th>
            <th class="hidden-phone">删除收藏的新闻</th>
        </tr>
		<%for(int i = 0;i < nc1.size(); i++ ){ %>
        <tr>
            <td><a href="UserServlet?opr=viewnews&newsid=<%=nc1.get(i).newsid%>"><%=nc1.get(i).newstitle %></font></td>
            <td><%=nc1.get(i).newstime %></td>

            <td class="hidden-phone"><%=nc1.get(i).newstheme %></td>
            <td class="hidden-phone"><%=authorname.get(i) %></td>
            <td class="hidden-phone"><a href="UserServlet?opr=dcln&newsid=<%=nc1.get(i).newsid%>">删除</a></td>
